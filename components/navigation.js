...
function Navigation({ flat = false, selectedKey = "home" }) {
	const MENU = [
		...
		{key: "bookrmarks", path: "/bookrmarks", title: "Bookmarks", icon: <Bookmark/>, iconSelected: <BookmarkFill/>},
	...
	return <nav className={styles.nav}>
		{MENU.map(menu => (
			<NavButton selected = {(router.pathname == menu.path)}
			href = {menu.path}
			notify = {menu.notify}
			className = {styles.navButton}>
				{selectedKey == menu.key ? menu.iconSelected : menu.icon}
